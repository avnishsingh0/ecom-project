...
import { listProductDetails } from "../Redux/SingleProduct/action";
const SingleProduct = ({  match }) => {
  const { id } = useParams();
  const [qty, setQty] = useState(1);
  const dispatch = useDispatch();
  const productDetails = useSelector((state) => state.singleReducer);
          ...
          <Box p={5} w={"90%"} border={"0px solid red"}>
            ...
            <HStack mt={7} mb={5}>
              <Link to={`/cart/${id}?qty=${qty}`}>
                <Button colorScheme="red" leftIcon={<AiOutlineShoppingCart />}>
                  ADD TO BAG
                </Button>
              </Link>
