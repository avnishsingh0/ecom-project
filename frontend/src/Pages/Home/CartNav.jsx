...
const CartNav = ({ step1, step2, step3, step4 }) => {
  const { id } = useParams();
  ...
  return (
    <>
      ...
      >
        ...
        <Nav.Item>
          {step1 ? (
            <LinkContainer to="/cart/bag">
              <Link ><Text color={"green"} fontSize={"15px"}>Bag</Text></Link>
            </LinkContainer>
          ) : (
            <Link disabled><Text ml={"10px"} fontSize={"15px"} color={"#a9a9a9"} >Bag</Text></Link>
          )}
        </Nav.Item>
        <Nav.Item>
          {step2 ? (
            <LinkContainer to="/cart/address">
              <Link><Text color={"green"}  fontSize={"15px"} ml={"10px"}>Address</Text></Link>
            </LinkContainer>
          ) : (
            <Link disabled><Text ml={"10px"} fontSize={"15px"} color={"#a9a9a9"}>Address</Text></Link>
          )}
        </Nav.Item>
        <Nav.Item>
          {step3 ? (
            <LinkContainer to="/cart/payment">
              <Link><Text color={"green"} fontSize={"15px"}  ml={"10px"}>Payment</Text></Link>
            </LinkContainer>
          ) : (
            <Link  disabled ><Text ml={"10px"} fontSize={"15px"} color={"#a9a9a9"}>Payment</Text></Link>
          )}
        </Nav.Item>
        <Nav.Item>
          {step4 ? (
            <LinkContainer to="cart/placeorder">
            <Link><Text color={"green"} fontSize={"15px"}  ml={"10px"}>PlaceOrder</Text></Link>
            </LinkContainer>
          ) : (
            <Link  disabled ><Text ml={"10px"} fontSize={"15px"} color={"#a9a9a9"}>PlaceOrder</Text></Link>
          )}
        </Nav.Item>
      </Nav>
       
          {/* <Link to={`/cart/${id}?qty=${qty}`}>Bag</Link>
